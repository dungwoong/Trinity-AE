...
(seq
    ...
    match list_expressions_with_target_cost_v3_part1(&runner, "/home/jhpark676/Project/TileIR/expressions/semi/keyformer_llama_cost6_kern2.json", 6, 2) {
        Ok(count) => println!("Saved {} expressions", count),
        Err(e) => eprintln!("Save error: {}", e),
    }

    let (expressions, tile_sets) = match list_expressions_from_semi_with_cost(&runner, "/home/jhpark676/Project/TileIR/expressions/semi/keyformer_llama_cost6_kern2.json", usize::MAX) {
    ...
    let file = File::create("/home/jhpark676/Project/TileIR/expressions/keyformer_llama_cost6_kern2.txt").expect("Failed to create file");
...
(seq
    ...
    match list_expressions_with_target_cost_v3_part1(&runner, "/home/jhpark676/Project/TileIR/expressions/semi/keyformer_falcon_cost6_kern2.json", 6, 2) {
        Ok(count) => println!("Saved {} expressions", count),
        Err(e) => eprintln!("Save error: {}", e),
    }

    let (expressions, tile_sets) = match list_expressions_from_semi_with_cost(&runner, "/home/jhpark676/Project/TileIR/expressions/semi/keyformer_falcon_cost6_kern2.json", usize::MAX) {
        Ok((expressions, tile_sets)) => {
            println!("Loaded {} final expressions", expressions.len());
            println!("{:?}", tile_sets);
            (expressions, tile_sets)
    ...
    let file = File::create("/home/jhpark676/Project/TileIR/expressions/keyformer_falcon_cost6_kern2.txt").expect("Failed to create file");
    let mut writer = BufWriter::new(file);
