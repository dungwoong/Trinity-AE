...
fn extract_rmsnorm_qkv_attn_expressions() {
    setup_shape_tracker(vec![
        ...
        ("K", vec![64, 16, 128]),
        ("V", vec![64, 16, 128]),
        ("K_cache", vec![64, 528, 128]),
        ("V_cache", vec![64, 528, 128]),
        ("C", vec![64, 16, 528]),
        ("C_exp", vec![64, 16, 528]),
        ("C_sum", vec![64, 16]),
        ("C_div", vec![64, 16, 528]),
...
";
    ...
    match list_expressions_with_target_cost_v3_part1(&runner, "/home/jhpark676/Project/TileIR/expressions/semi/llama70b_rmsnorm_qkv_attn_cost6_kern2.json", 6, 2) {
        Ok(count) => println!("Saved {} expressions", count),
        Err(e) => eprintln!("Save error: {}", e),
    }

    let (expressions, tile_sets) = match list_expressions_from_semi_with_cost(&runner, "/home/jhpark676/Project/TileIR/expressions/semi/llama70b_rmsnorm_qkv_attn_cost6_kern2.json", usize::MAX) {
    ...
    let file = File::create("/home/jhpark676/Project/TileIR/expressions/llama70b_rmsnorm_qkv_attn_cost6_kern2.txt").expect("Failed to create file");
...
fn extract_ffn_expressions() {
    setup_shape_tracker(vec![
        ...
        ("attn_O2", vec![16, 8192]),
        ("attn_O3", vec![16]),
        ("attn_O_norm", vec![16, 8192]),
        ("WFF1a", vec![8192, 28672]),
        ("WFF1b", vec![8192, 28672]),
        ("FF1a", vec![16, 8192]),
        ("FF1b", vec![16, 8192]),
        ("FF1b_silu", vec![16, 28672]),
        ("FF1", vec![16, 28672]),
        ("FF2", vec![16, 8192]),
        ("WFF2", vec![28672, 8192]),
        ("O_FF", vec![16, 8192]),
...
(seq
    ...
    match list_expressions_with_target_cost_v3_part1(&runner, "/home/jhpark676/Project/TileIR/expressions/semi/llama70b_ffn_cost6_kern5_wo_scheduler2.json", 6, 5) {
        Ok(count) => println!("Saved {} expressions", count),
        Err(e) => eprintln!("Save error: {}", e),
    }

    let (expressions, tile_sets) = match list_expressions_from_semi_with_cost(&runner, "/home/jhpark676/Project/TileIR/expressions/semi/llama70b_ffn_cost6_kern5_wo_scheduler2.json", usize::MAX) {
        Ok((expressions, tile_sets)) => {
            println!("Loaded {} final expressions", expressions.len());
            println!("{:?}", tile_sets);
            (expressions, tile_sets)
    ...
    let file = File::create("/home/jhpark676/Project/TileIR/expressions/llama70b_ffn_cost6_kern5_wo_scheduler2.txt").expect("Failed to create file");
    let mut writer = BufWriter::new(file);
