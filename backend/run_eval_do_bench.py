...
def main():
    ...
    if len(baseline) == 0 or "trinity" in baseline:
        ...
        trinity_time = do_bench(lambda: forward(*args))
        time.sleep(2)
    ...
    if len(baseline) == 0 or "tensorrt" in baseline:
        ...
        with torch.no_grad():
            ...
            tensorrt_time = do_bench(lambda: trt(*inputs))
            time.sleep(2)
    ...
    if len(baseline) == 0 or "pytorch" in baseline:
        ...
        with torch.no_grad():
            ...
            torcheager_time = do_bench(lambda: ti(*inputs))
            time.sleep(2)
    ...
    if len(baseline) == 0 or "inductor" in baseline:
        ...
        inductor_time = do_bench(lambda: compiled_model(*inputs))
        time.sleep(2)
