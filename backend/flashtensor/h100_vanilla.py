...
def bench_vanilla(model, M, N, P, D, H, device, dtype):
    ...
    for _ in range(10):
        vanilla_kernel(x, wqkv, k_cache, v_cache, M, N, P, D, H, attn)
...
def vanilla_kernel(x, wqkv, k_cache, v_cache, M, N, P, D, H, attn):
...
def bench_Attn_p3_llama():
  dev = torch.cuda.current_device()
  # q (1,16,32,128), k/v (1,1024,32,128)
  rand_arg_0 = torch.randn(1, 16, 32, 128, dtype=torch.float16, device=dev)
  rand_arg_1 = torch.randn(1, 1024, 32, 128, dtype=torch.float16, device=dev)  # v
  rand_arg_2 = torch.randn(1, 1024, 32, 128, dtype=torch.float16, device=dev)  # k
  avg_ms = triton.testing.do_bench(lambda: Attn_p3_llama(rand_arg_0, rand_arg_1, rand_arg_2))
  # print('[Attn_p3] avg_ms:', avg_ms)
  return avg_ms

def Attn_p3_llama(arg0: torch.Tensor, arg1: torch.Tensor, arg2: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
  dev = arg0.device
...
def bench_Attn_p3_falcon():
  dev = torch.cuda.current_device()
  # q (1,16,71,64), k/v (1,1024,71,64)
  rand_arg_0 = torch.randn(1, 16, 71, 64, dtype=torch.float16, device=dev)
  rand_arg_1 = torch.randn(1, 1024, 71, 64, dtype=torch.float16, device=dev)  # v
  rand_arg_2 = torch.randn(1, 1024, 71, 64, dtype=torch.float16, device=dev)  # k
  avg_ms = triton.testing.do_bench(lambda: Attn_p3_falcon(rand_arg_0, rand_arg_1, rand_arg_2))
  print('[Attn_p3] avg_ms:', avg_ms)

def Attn_p3_falcon(arg0: torch.Tensor, arg1: torch.Tensor, arg2: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
  dev = arg0.device
  autotune_key = torch.cuda.get_device_capability(dev)[0]
  q = arg0
  v = arg1
