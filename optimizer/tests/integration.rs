...
";
    ...
    match list_expressions_with_target_cost_v3_part1(
        &runner,
        "/home/jhpark676/Project/trinity/expressions/semi/attn_only.json",
        2,
        2,
    ) {
...
";
    ...
    let cost_expressions = list_expressions_with_target_cost(&runner);
    println!(
        "[Cost model] There are {:?} expressions",
        cost_expressions.len()
    );

    let cost_expressions_v2 = list_expressions_with_target_cost_v2(&runner);
    println!(
        "[Cost model v2] There are {:?} expressions",
        cost_expressions_v2.len()
    );
...
fn extract_lora() {
    ...
    let expressions = match list_expressions_from_semi_all(
        &runner,
        "/home/jhpark676/Project/trinity/expressions/semi/lora_cost3_kern2.json",
        0,
    ) {
        Ok(expressions) => {
    ...
    let file =
        File::create("/home/jhpark676/Project/trinity/expressions/lora_cost3_kern2_index0.txt")
...
fn extract_gated_mlp_skip_ft() {
    ...
    let expressions = match list_expressions_from_semi_all(
        &runner,
        "/home/jhpark676/Project/trinity/expressions/semi/gated_mlp_skipft_cost3_kern2.json",
        17,
    ) {
        Ok(expressions) => {
            println!("Loaded {} final expressions", expressions.len());
    ...
    let file = File::create(
        "/home/jhpark676/Project/trinity/expressions/gated_mlp_skipft_cost3_kern2_index17.txt",
    )
    .expect("Failed to create file");
    let mut writer = BufWriter::new(file);
...
";
    ...
    let (expressions, tile_sets) = match list_expressions_from_semi_with_cost(
        &runner,
        "/home/jhpark676/Project/trinity/expressions/semi/attacc_cost3_kern2.json",
        usize::MAX,
    ) {
    ...
    let file = File::create(
        "/home/jhpark676/Project/trinity/expressions/attacc_cost3_kern2_with_cost.txt",
    )
    .expect("Failed to create file");
...
";
    ...
    match list_expressions_with_target_cost_v3_part1(
        &runner,
        "/home/jhpark676/Project/trinity/expressions/semi/rmsnorm_cost3_kern1.json",
        3,
        1,
    ) {
        Ok(count) => println!("Saved {} expressions", count),
    ...
    let file = File::create("/home/jhpark676/Project/trinity/expressions/rmsnorm_cost3_kern1.txt")
        .expect("Failed to create file");
    // let file = File::create("tmp.txt").expect("aa");
    let mut writer = BufWriter::new(file);
...
fn visualizer() {
    ...
    ";
    let runner = run_until_saturated(expr, custom_rules(), 10);
