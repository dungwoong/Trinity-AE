...
use crate::utils::*;
use crate::cost::*;
use crate::shape::{TensorShape, ShapeTracker, Dimension};
        ...
        match enode {
            ...
            TileLang::Load([base, idx]) => {
                ...
                let tensor_shape = SHAPE_TRACKER.with(|tracker| {
                    ...
                    if let Some(base_name) = get_base_name_egraph(egraph, *base) {
                        ...
                        let first_tensor_name = base_name.split(',').next().unwrap_or(&base_name).trim();
            ...
            TileLang::Add([left, right]) => {
                ...
                let tensor_shape = match (&x(left).tensor_shape, &x(right).tensor_shape) {
                    (Some(left_shape), Some(right_shape)) if left_shape.dims.len() == right_shape.dims.len() => {
                    ...
                    (Some(left_shape), Some(right_shape)) => {
                        ...
                        egraph[*axis].nodes.iter().find_map(|n| match n {
                            TileLang::Num(axis_val) => {
                                ...
                                if axis_idx < left_shape.dims.len() && left_shape.dims.len() == right_shape.dims.len() {
                                    let mut result_dims = Vec::new();
                                    ...
                                    for i in 0..left_shape.dims.len() {
                                        if i == axis_idx {
                                            ...
                                            match (&left_shape.dims[i], &right_shape.dims[i]) {
                                                ...
                                                (Dimension::Wildcard, _) | (_, Dimension::Wildcard) => {
            ...
            TileLang::Permute3([input, axis0, axis1, axis2]) => {
                ...
                let tensor_shape = x(input).tensor_shape.as_ref().and_then(|input_shape| {
                    if input_shape.dims.len() >= 3 {
                        ...
                        let axes: Vec<Option<usize>> = vec![axis0, axis1, axis2].into_iter().map(|axis_id| {
...
fn extract_tile_dims(tracker: &ShapeTracker, egraph: &EGraph, idx_id: &Id, base_shape: &TensorShape) -> Vec<Dimension> {
...
fn extract_tile_dims_helper(
    ...
    if let Some(index_node) = egraph[*idx_id].nodes.iter().find(|n| matches!(n, TileLang::Index(_))) {
        if let TileLang::Index(tile_indices) = index_node {
            for tile_id in tile_indices.iter() {
                if *current_dim < base_shape.dims.len() {
                    ...
                    if egraph[*tile_id].nodes.iter().any(|n| matches!(n, TileLang::Index(_))) {
                        // Nested index - recursively process without incrementing current_dim yet
                        let nested_dims = extract_tile_dims_helper(tracker, egraph, tile_id, base_shape, current_dim);
                        ...
                        match base_dim {
                            Dimension::Concrete(full_dim) => {
                                ...
                                let tile_elem_nodes: Vec<_> = egraph[*tile_id].nodes.iter()
                                ...
                                let tile_dim = if tile_elem_nodes.len() > 1 {
                                    ...
                                    if loop_vars.len() > 1 {
                                        ...
                                        egraph[*tile_id].nodes.iter().find_map(|n| match n {
                                            TileLang::Tile(_) => Some(Dimension::Concrete(tracker.get_tile_size(*full_dim))),
                                    ...
                                    egraph[*tile_id].nodes.iter().find_map(|n| match n {
                                        ...
                                        TileLang::ConstTile([_, interval_id]) => {
                                            egraph[*interval_id].nodes.iter().find_map(|interval_node| match interval_node {
                                                TileLang::Num(n) => Some(Dimension::Concrete(*n as usize)),
                                                _ => None,
                                            })
                                        },
                                        TileLang::Elem(_) => Some(Dimension::Concrete(1)),
