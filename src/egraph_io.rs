use egg::*;
use crate::language::{TileLang, LoopAnalysis};
use std::fs::File;
...
pub fn save_raw_egraph(
    ...
    writeln!(file, "# Total nodes: {}", runner.egraph.total_number_of_nodes())?;
    writeln!(file, "# Total classes: {}", runner.egraph.number_of_classes())?;
    ...
    println!("Saved raw egraph with {} classes to {}", 
             runner.egraph.number_of_classes(), filepath);
    
    Ok(())
}

// Helper function to format a node with ID references instead of expanded expressions
fn format_node_with_ids(
    egraph: &EGraph<TileLang, LoopAnalysis>,
    node: &TileLang,
) -> String {
    use std::fmt::Write;
    use TileLang::*;
...
pub fn load_raw_egraph(
    ...
    for line in reader.lines() {
        ...
        if line.starts_with("CLASS ") {
            ...
            if parts.len() >= 3 {
                if let (Ok(id), Ok(canonical_id)) = (parts[1].parse::<usize>(), parts[2].parse::<usize>()) {
    ...
    println!("Loaded egraph with {} classes from {}", 
             egraph.number_of_classes(), filepath);
...
fn parse_node(
    ...
    if !node_str.starts_with('(') {
        ...
        let node = match op {
            "loop" if child_ids.len() >= 5 => {
                Loop([child_ids[0], child_ids[1], child_ids[2], child_ids[3], child_ids[4]])
            }
            "seq" if child_ids.len() >= 2 => {
                Seq([child_ids[0], child_ids[1]])
            }
            "store" if child_ids.len() >= 3 => {
                Store([child_ids[0], child_ids[1], child_ids[2]])
            }
            "load" if child_ids.len() >= 2 => {
                Load([child_ids[0], child_ids[1]])
            }
            "+" if child_ids.len() >= 2 => {
                Add([child_ids[0], child_ids[1]])
            }
            "x" if child_ids.len() >= 2 => {
                Mul([child_ids[0], child_ids[1]])
            }
            "*" if child_ids.len() >= 2 => {
                Matmul([child_ids[0], child_ids[1]])
            }
            "index" => {
                // index takes a Box<[Id]>
                Index(child_ids.clone().into_boxed_slice())
            }
            "tile" if child_ids.len() >= 1 => {
                Tile(child_ids[0])
            }
            "input" if child_ids.len() >= 1 => {
                Input(child_ids[0])
            }
            "output" if child_ids.len() >= 1 => {
                Output(child_ids[0])
            }
            "tensor" if child_ids.len() >= 1 => {
                Tensor(child_ids[0])
            }
            "const_tile" if child_ids.len() >= 2 => {
                ConstTile([child_ids[0], child_ids[1]])
            }
            "fulltile" => {
                FullTile
            }
            "elem" if child_ids.len() >= 1 => {
                Elem(child_ids[0])
            }
            "const" if child_ids.len() >= 1 => {
                Const(child_ids[0])
            }
            "-" if child_ids.len() >= 2 => {
                Sub([child_ids[0], child_ids[1]])
            }
            "/" if child_ids.len() >= 2 => {
                Div([child_ids[0], child_ids[1]])
            }
            "exp" if child_ids.len() >= 1 => {
                Exp(child_ids[0])
            }
            "rsum" if child_ids.len() >= 2 => {
                ReduceSum([child_ids[0], child_ids[1]])
            }
            "sqr" if child_ids.len() >= 1 => {
                Sqr(child_ids[0])
            }
            "sqrt" if child_ids.len() >= 1 => {
                Sqrt(child_ids[0])
            }
            "sigmoid" if child_ids.len() >= 1 => {
                Sigmoid(child_ids[0])
            }
            "concat" if child_ids.len() >= 3 => {
                Concat([child_ids[0], child_ids[1], child_ids[2]])
            }
            "bcast" if child_ids.len() >= 2 => {
                Broadcast([child_ids[0], child_ids[1]])
            }
            "permute3" if child_ids.len() >= 4 => {
                Permute3([child_ids[0], child_ids[1], child_ids[2], child_ids[3]])
            }
            "squeeze" if child_ids.len() >= 2 => {
                Squeeze([child_ids[0], child_ids[1]])
            }
            "unsqueeze" if child_ids.len() >= 2 => {
                Unsqueeze([child_ids[0], child_ids[1]])
            }
            "dummy" => {
                Dummy
            }
            "dloop" if child_ids.len() >= 5 => {
                DLoop([child_ids[0], child_ids[1], child_ids[2], child_ids[3], child_ids[4]])
            }
            "tmp_loop" if child_ids.len() >= 6 => {
                TLoop([child_ids[0], child_ids[1], child_ids[2], child_ids[3], child_ids[4], child_ids[5]])
            }
            "sloop" if child_ids.len() >= 5 => {
                SLoop([child_ids[0], child_ids[1], child_ids[2], child_ids[3], child_ids[4]])
            }
            "ploop" if child_ids.len() >= 5 => {
                PLoop([child_ids[0], child_ids[1], child_ids[2], child_ids[3], child_ids[4]])
            }
            _ => {
                eprintln!("Unknown operator or wrong arity: {} with {} children", op, child_ids.len());
                return None;
            }
        };
